...
const Snow = ({ position = [0, -0.75, 0], radius, intensity, windSpeed }) => {
  ...
  useEffect(() => {
    ...
    if (snowRef.current) {
      ...
      geometry.setAttribute("position", new THREE.BufferAttribute(snowArray, 3));
      snowRef.current.geometry = geometry; // Assign geometry to the ref
    }
  }, [snowArray]);
  ...
  useFrame(() => {
    if (snowRef.current) {
      ...
      if (snowRef.current.geometry && snowRef.current.geometry.attributes.position) {
